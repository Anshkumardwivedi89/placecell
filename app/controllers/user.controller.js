    ...
    if (!_b.password || !_b.token) {
      ...
      try {
        ...
        if (!user) {
          ...
          const hashedPassword = await bcrypt.hash(_b.password, salt);
...
exports.changePassword = async (req, res) => {
    const _b = req.body;
    console.log(_b.new_password);

    if(!_b.old_password || !_b.new_password) {
        res.status(200).json({ success : false, message : 'Old or new password is missing.'})
    } else {
        try {
            const user = await User.findOne({ college_id : req.decoded.college_id }).select('college_id student_name college_email password');

            if(!user) {
                res.status(200).json({ success : false, message : 'User not found.'});
            } else {
                let validPassword = await user.comparePassword(_b.old_password);
                

                if(validPassword) {
                    // Hash the new password before saving

                    bcrypt.hash(_b.new_password, 10, async (err, hashed) => {
                        if (err) {
                            // Handle error
                            console.error(err);
                        ...
                        console.log(hashed); // Log the hashed password
                   ...
                   // const hashedPassword = await bcrypt.hash(_b.new_password, 10);
                   // user.password = hashedPassword;
